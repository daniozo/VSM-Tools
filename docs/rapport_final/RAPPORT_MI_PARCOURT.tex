...
\usepackage[french]{babel} % Pour la typographie française et les noms (Table des matières, Figure)
...
\hypersetup{
    ...
    filecolor=magenta,
    urlcolor=cyan,
    pdftitle={Rapport de Présentation : VSM-Tools},
    pdfpagemode=FullScreen,
}

\title{Rapport de Présentation : VSM-Tools - Optimisation des Flux de Valeur}
\author{Équipe Projet VSM-Tools} % Mettez les noms des auteurs ici si vous préférez
\date{\today} % Utilise la date de compilation
...
\begin{itemize}
    ...
    \item \textbf{Un élève-ingénieur spécialisé en Intelligence Artificielle et Bases de Données (IABD) :} Responsable des aspects techniques du projet (conception de l'architecture logicielle, choix technologiques, conception des fonctionnalités avancées IA/prédictives, gestion des données, coordination technique).
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Sprint 6 : Conception UI/UX (Mockups) \& Modèle de Données :} Maquettes haute fidélité, structure des données VSM (JSON/TypeScript), API Electron.
        \item \textbf{Sprint 7 : Finalisation Documentation de Conception :} Consolidation et validation des documents (\texttt{SYSTEME\_DESIGN.md}, \texttt{INTERFACE\_UTILISATEUR.md}, exigences).
...
\begin{itemize}
    \item \textbf{Cadrage et Recherche (Sprints 1 \& 2) :} Le périmètre du projet a été défini, les rôles établis, et une recherche approfondie sur la méthodologie VSM et les outils concurrents a été menée. Les synthèses correspondantes (\texttt{vsm\_et\_indicateurs.md}, analyse concurrentielle dans Section 3) ont été produites.
    ...
    \item \textbf{Conception Initiale (Sprints 4 \& 5) :} Les exigences non-fonctionnelles ont été listées. La conception de l'interface utilisateur a débuté avec la création de wireframes (\texttt{INTERFACE\_UTILISATEUR.md}). L'architecture système globale et les choix technologiques principaux ont été définis et documentés (\texttt{SYSTEME\_DESIGN.md}).
    \item \textbf{Conception Détaillée (Sprints 6 \& 7) :} Les maquettes haute fidélité (mockups) ont été créées, affinant la vision de l'interface utilisateur. Le modèle de données précis pour les cartes VSM et l'API Electron a été spécifié. La documentation de conception (\texttt{SYSTEME\_DESIGN.md}, \texttt{INTERFACE\_UTILISATEUR.md}) a été consolidée.
\end{itemize}
...
\begin{itemize}
    ...
    \item Un ensemble de documents de conception détaillés (exigences, architecture, UI/UX, modèle de données) constituant une base solide pour la future phase de développement.
...
VSM-Tools se positionne comme une réponse pragmatique et puissante à ces défis. Il combine une interface intuitive pour la modélisation des flux avec des capacités de calcul automatique des indicateurs clés (Lead Time, \%VA...) et, de manière distinctive, des \textbf{fonctionnalités d'analyse avancées intégrées} : simulation de scénarios \og what-if \fg{}, éléments d'analyse prédictive et suggestions d'optimisation. L'étude comparative a mis en évidence cet équilibre entre accessibilité et puissance analytique comme un différenciateur clé.
...
\section{Annexes}
\label{sec:annexes}
\newpage % Commence les annexes sur une nouvelle page
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../images/plan_daction.png}
